...
/**
 * @param { width: 'full' | 'md', isOpen: bolean } param0
 * @returns
 */

export function UIModal({ width = 'md', isOpen = false,  onClose, className, children,}) {
  ...
  const modal = (
    <div 
      className={clsx(
        "fixed inset-0 bg-slate-900/60 backdrop-blur pt-10 overflow-y-auto",
        className
      )}
    ...
    >
      <div
        data-id='modal'
        className={clsx(
          'bg-white rounded-lg m-auto relative p-6 min-h-[320px] flex flex-col',
          {
      ...
      >
        <button 
          onClick={onClose}
          className='w-8 h-8 rounded bg-white/10 hover:bg-white/40 transition-colors active:scale-95 absolute -right-11 top-0'><CrossLigthIcon className='m-auto'/
        ></button>
        {children}
      </div>
    </div>
...
UIModal.Footer = function UIModalFooter({children, className}){
  return(
    <div className={clsx(
      'flex gap-4 justify-end mt-auto',
      className
    )} >{children}</div>
  )
}
